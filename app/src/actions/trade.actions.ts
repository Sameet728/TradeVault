...
function toPlainTrade(doc: ITrade) {
  ...
  return {
    ...
    tradeDate: obj.tradeDate.toISOString(),
...
export async function getTradesAction(filters: TradeFilters = {}): Promise<TradePaginated> {
  ...
  return {
    trades: docs.map((d) => ({
      ...
      tradeDate: d.tradeDate instanceof Date ? d.tradeDate.toISOString() : String(d.tradeDate),
      closeDate: d.closeDate instanceof Date ? d.closeDate.toISOString() : undefined,
